...
*   **Clarity:** Use diagrammatic elements like arrows and labels when they help explain relationships or flows. Text should be clear and legible.
...
*   **Output SVG Only:** Your entire response must be ONLY the raw SVG code. Start with \`<svg ...>\` and end with \`</svg>\`. Do not include explanations, notes, or markdown formatting.
...
${description}`;
    ...
    if (svgMatch && svgMatch[0].includes('hsl(var')) {
        return svgMatch[0];
    }
...
export async function simplifyTopicAsAnimation(input: SimplifyTopicInput): Promise<SimplifyTopicAnimationOutput> {
  ...
  const scenePromises = script.scenes.map(async (sceneDescription) => {
    try {
        ...
        console.error(`Failed to generate SVG for scene: ${sceneDescription}`, error);
        ...
        return { description: sceneDescription, svg: errorSvg };
    }
  });
